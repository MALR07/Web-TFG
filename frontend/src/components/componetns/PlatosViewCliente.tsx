...
const HistorialReservas: React.FC = () => { 
  ...
  const getEstadoColor = (estado: string) => { 
    switch (estado) { 
      ...
      case "presentado": 
        return "text-yellow-600"; // Mostaza 
      default: 
        return "text-gray-600"; // Por defecto gris 
    } 
  }; 

  // Función para restar 2 horas a la fecha de la reserva 
  ...
  return ( 
    <div 
      className="max-w-full mx-auto p-6 bg-cover bg-fixed bg-center relative" 
      style={{ backgroundImage: "url('fondoreserva.jpeg')" }} 
    > 
      <div 
        className="bg-white text-yellow-400 p-4 rounded-md shadow-md mb-6 w-full text-center" 
      ...
      {reservas.length === 0 ? ( 
        ...
        <div 
          className="bg-gray-800 bg-opacity-60 p-6 rounded-lg shadow-lg mb-8" 
                ...
                <li key={reserva.id_reserva} className="border rounded-md p-4 shadow-sm bg-white">
                  <div className="flex justify-between items-center">
                    <div>
                      ...
                      <p>Estado: <span className={`font-semibold ${getEstadoColor(reserva.estado)}`}>{reserva.estado}</span></p>
                    </div>
                    <button 
                  ...
                  <div 
                    className={`mt-4 overflow-hidden max-h-0 transition-all duration-500 ease-in-out ${expandedReserva === reserva.id_reserva ? "max-h-screen" : ""}`}
                  >
                    {plato && (
                      <div className="mt-4">
                        {plato.imagen && (
                        ...
                        {comentarioExistente ? (
                          <div className="bg-gray-50 p-4 rounded shadow-sm mt-4">
                            ...
                            {comentarioExistente.rating !== undefined && (
                              <div className="mb-2">{renderStars(comentarioExistente.rating)}</div>
                            )}
